//! Domain policy for DigestTipUser aggregate
//!
...
/// Domain policy for DigestTipUser — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<DigestTipUser>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type DigestTipUserDomainPolicy = PermitAllPolicy<DigestTipUser>;
