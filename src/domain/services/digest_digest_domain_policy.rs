//! Domain policy for DigestDigest aggregate
//!
...
/// Domain policy for DigestDigest — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<DigestDigest>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type DigestDigestDomainPolicy = PermitAllPolicy<DigestDigest>;
